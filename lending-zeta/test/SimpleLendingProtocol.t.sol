...
contract SimpleLendingProtocolTest is Test {
    ...
    function setUp() public {
        ...
        lendingProtocol = new SimpleLendingProtocol(payable(address(gateway)), owner);
    ...
    function testSupply() public {
        ...
        lendingProtocol.supply(address(ethToken), supplyAmount);
        vm.stopPrank();
        
        assertEq(lendingProtocol.userSupplies(user1, address(ethToken)), supplyAmount);
    }

    function testSupplyUnsupportedAsset() public {
        vm.prank(user1);
        vm.expectRevert("Asset not supported");
        lendingProtocol.supply(address(0x999), 1000);
    }

    function testSupplyZeroAmount() public {
        vm.prank(user1);
        vm.expectRevert("Amount must be greater than 0");
        lendingProtocol.supply(address(ethToken), 0);
    }
    ...
    function testCrossChainSupply() public {
        ...
        lendingProtocol.onCall(context, address(ethToken), supplyAmount, message);
    ...
    function testBorrow() public {
        ...
        assertEq(lendingProtocol.userBorrows(user1, address(usdcToken)), borrowAmount);
    ...
    function testRepay() public {
        ...
        lendingProtocol.repay(address(usdcToken), repayAmount);
        vm.stopPrank();
        
        assertEq(lendingProtocol.userBorrows(user1, address(usdcToken)), borrowAmount - repayAmount);
    }
    ...
    function testCrossChainRepay() public {
        ...
        lendingProtocol.onCall(context, address(usdcToken), repayAmount, message);
    ...
    function testWithdraw() public {
        ...
        lendingProtocol.withdraw(address(ethToken), withdrawAmount);
        vm.stopPrank();
        
        assertEq(lendingProtocol.userSupplies(user1, address(ethToken)), supplyAmount - withdrawAmount);
    ...
    function testLiquidation() public {
        ...
        emit Liquidate(liquidator, user2, repayAmount, expectedCollateral);
        
        lendingProtocol.liquidate(user2, address(ethToken), address(usdcToken), repayAmount);
        vm.stopPrank();
        
        assertEq(lendingProtocol.userBorrows(user2, address(usdcToken)), borrowAmount - repayAmount);
    }
    ...
    function testCannotLiquidateHealthyPosition() public {
        ...
        lendingProtocol.liquidate(user2, address(ethToken), address(usdcToken), 100 * 10**6);
        vm.stopPrank();
    }
    ...
    function testHealthFactorCalculation() public {
        ...
        assertEq(healthFactor, 400); // 400% = $4000 / $1000 * 100
    }
    ...
    function testCollateralValue() public {
        ...
        uint256 collateralValue = lendingProtocol.getCollateralValue(user1, address(ethToken));
    ...
    function testDebtValue() public {
        ...
        uint256 debtValue = lendingProtocol.getDebtValue(user1, address(usdcToken));
    ...
    function testCanBorrow() public {
        ...
        assertTrue(lendingProtocol.canBorrow(user1, address(usdcToken), 2000 * 10**6));
        assertFalse(lendingProtocol.canBorrow(user1, address(usdcToken), 3000 * 10**6));
    }
    ...
    function testCanWithdraw() public {
        ...
        assertTrue(lendingProtocol.canWithdraw(user1, address(ethToken), 0.5 * 10**18));
        assertFalse(lendingProtocol.canWithdraw(user1, address(ethToken), 2 * 10**18));
    }
    ...
    function testMultipleAssetsCollateralAndDebt() public {
        ...
        lendingProtocol.supply(address(ethToken), ethSupply);
        
        usdcToken.approve(address(lendingProtocol), usdcSupply);
        lendingProtocol.supply(address(usdcToken), usdcSupply);
        
        // Borrow both assets
        lendingProtocol.borrow(address(ethToken), ethBorrow);
        lendingProtocol.borrow(address(usdcToken), usdcBorrow);
        
        vm.stopPrank();
        
        // Check total values
        uint256 totalCollateral = lendingProtocol.getTotalCollateralValue(user1);
        ...
        assertEq(lendingProtocol.getHealthFactor(user1), 250); // 250%
    }
