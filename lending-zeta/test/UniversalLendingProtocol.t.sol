...
contract UniversalLendingProtocolTest is Test {
    ...
    function testCannotLiquidateHealthyPosition() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalLendingProtocol.HealthFactorTooLow.selector
            )
        );
        lendingProtocol.liquidate(
            user2,
    ...
    function testOnlyGatewayCanCallOnCall() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalLendingProtocol.Unauthorized.selector
            )
        );
    ...
    function testWithdrawBreaksHealthFactor() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalLendingProtocol.InsufficientCollateral.selector
            )
        );
        lendingProtocol.withdraw(address(ethToken), withdrawAmount, user1);
        vm.stopPrank();
    ...
    function testZeroAmountOperations() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalLendingProtocol.InvalidAmount.selector
            )
        );
    ...
    function testUnsupportedAssetOperations() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalLendingProtocol.AssetNotSupported.selector,
                unsupportedAsset
            )
        );
    ...
    function testOnRevert() public {
        ...
        lendingProtocol.onRevert(revertContext, address(ethToken), 1000000, "");
    }
    ...
    function testCrossDecimalLiquidation() public {
        ...
        uint256 userSolSupply = lendingProtocol.userSupplies(
            user1,
            address(solToken)
        );
        uint256 userEthBorrow = lendingProtocol.userBorrows(
            user1,
            address(ethToken)
        );
        ...
        (
            ...
            uint256 liquidatedCollateral,
            bool canLiquidate
        ) = lendingProtocol.getMaxLiquidation(
                user1,
                address(solToken),
                address(ethToken)
            );
        ...
        assertTrue(
            ...
            "Liquidated collateral should be > 0"
        );
        assertTrue(
            liquidatedCollateral <= userSolSupply,
            "Cannot liquidate more than available"
        );

        // The max repay amount should be reasonable (not tiny like 0.000000000238142857)
        assertTrue(
            maxRepayAmount > 1e15,
            "Max repay amount should be reasonable (> 0.001 ETH)"
        );
        ...
        uint256 solReceived = liquidatorSolBalanceAfter -
            liquidatorSolBalanceBefore;
        uint256 ethPaid = liquidatorEthBalanceBefore -
            liquidatorEthBalanceAfter;
        ...
        assertEq(
            ethPaid,
            maxRepayAmount,
            "ETH paid should match max repay amount"
        );
        ...
        assertTrue(
            solReceivedUsdValue > ethPaidUsdValue,
            "Liquidator should receive liquidation bonus"
        );

        vm.stopPrank();

        // Verify user's position is updated
        uint256 userSolSupplyAfter = lendingProtocol.userSupplies(
            user1,
            address(solToken)
        );
        uint256 userEthBorrowAfter = lendingProtocol.userBorrows(
            user1,
            address(ethToken)
        );

        assertTrue(
            userSolSupplyAfter < userSolSupply,
            "User SOL supply should decrease"
        );
        assertTrue(
            userEthBorrowAfter < userEthBorrow,
            "User ETH debt should decrease"
        );
    ...
    function testUsdcEthLiquidation() public {
        ...
        uint256 ethBorrowAmount = 0.5 * 10 ** 18; // 0.5 ETH (18 decimals) = $1000, safe amount
        ...
        console.log(
            "USDC/ETH Liquidated Collateral (USDC):",
        ...
        uint256 userUsdcSupplyAfter = lendingProtocol.userSupplies(
            user2,
            address(usdcToken)
        );
        uint256 userEthBorrowAfter = lendingProtocol.userBorrows(
        ...
        assertTrue(
            userUsdcSupplyAfter < usdcSupplyAmount,
            "User USDC supply should decrease"
        );
        assertTrue(
            userEthBorrowAfter < ethBorrowAmount,
            "User ETH debt should decrease"
        );
    }
}
