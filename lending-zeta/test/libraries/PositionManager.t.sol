...
contract PositionManagerTest is Test {
    ...
    mapping(address => IUniversalLendingProtocol.AssetConfig) public enhancedAssets;
    ...
    function testGetUserPositionData_SingleAsset() public {
        ...
        assertEq(positionData.totalCollateralValue, 1000e18, "Should calculate total collateral value");
        assertEq(positionData.totalDebtValue, 500e18, "Should calculate total debt value");
        assertEq(positionData.maxBorrowUsdValue, 100e18, "Should calculate max additional borrow");
        assertEq(positionData.healthFactor, 1.9e18, "Should calculate health factor");
        
        // Check arrays
        assertEq(positionData.suppliedAssets.length, 1, "Should have one supplied asset");
        assertEq(positionData.borrowedAssets.length, 1, "Should have one borrowed asset");
        assertEq(positionData.suppliedAssets[0], address(usdc), "Should identify supplied asset");
        assertEq(positionData.borrowedAssets[0], address(usdc), "Should identify borrowed asset");
        assertEq(positionData.suppliedAmounts[0], 1000e6, "Should report supplied amount");
        assertEq(positionData.borrowedAmounts[0], 500e6, "Should report borrowed amount");
    }
    ...
    function testGetUserPositionData_MultiAsset() public {
        ...
        assertEq(positionData.totalCollateralValue, 4000e18, "Should sum all collateral values");
        assertEq(positionData.totalDebtValue, 2000e18, "Should sum all debt values");
        assertEq(positionData.suppliedAssets.length, 2, "Should have two supplied assets");
        assertEq(positionData.borrowedAssets.length, 2, "Should have two borrowed assets");

        // Check weighted liquidation threshold calculation
        // Expected: (2000*0.95 + 2000*0.85) / 4000 = (1900 + 1700) / 4000 = 0.9
        assertEq(positionData.liquidationThreshold, 0.9e18, "Should calculate weighted liquidation threshold");
    }

    function testGetUserPositionData_NoPositions() public {
        PositionManager.UserPositionData memory positionData = PositionManager.getUserPositionData(
            USER1,
            supportedAssets,
            userSupplies,
        ...
        assertEq(positionData.totalCollateralValue, 0, "Should be zero with no positions");
        assertEq(positionData.totalDebtValue, 0, "Should be zero with no debt");
        assertEq(positionData.healthFactor, type(uint256).max, "Should have max health factor with no debt");
        assertEq(positionData.suppliedAssets.length, 0, "Should have no supplied assets");
        assertEq(positionData.borrowedAssets.length, 0, "Should have no borrowed assets");
    }
    ...
    function testGetUserPositionData_SupplyOnly() public {
        ...
        assertEq(positionData.totalCollateralValue, 4000e18, "Should calculate collateral value");
        assertEq(positionData.totalDebtValue, 0, "Should have no debt");
        assertEq(positionData.healthFactor, type(uint256).max, "Should have max health factor");
        assertEq(positionData.maxBorrowUsdValue, 3200e18, "Should calculate max borrowable: 4000*0.8");
        assertEq(positionData.suppliedAssets.length, 1, "Should have one supplied asset");
        assertEq(positionData.borrowedAssets.length, 0, "Should have no borrowed assets");
    }
    ...
    function testGetAssetPosition_WithBalance() public {
        ...
        PositionManager.AssetPosition memory position = PositionManager.getAssetPosition(
            USER1,
            address(usdc),
            userSupplies,
            userBorrows,
        ...
        assertEq(position.asset, address(usdc), "Should set correct asset address");
        assertEq(position.suppliedAmount, 1000e6, "Should report supplied amount");
        assertEq(position.borrowedAmount, 300e6, "Should report borrowed amount");
        assertEq(position.suppliedValue, 1000e18, "Should calculate supplied value");
        assertEq(position.borrowedValue, 300e18, "Should calculate borrowed value");
        assertEq(position.collateralValue, 900e18, "Should calculate effective collateral: 1000*0.9");
    }

    function testGetAssetPosition_NoBalance() public {
        ...
        assertEq(position.asset, address(usdc), "Should set correct asset address");
        assertEq(position.suppliedAmount, 0, "Should report zero supplied amount");
        assertEq(position.borrowedAmount, 0, "Should report zero borrowed amount");
        assertEq(position.suppliedValue, 0, "Should report zero supplied value");
        assertEq(position.borrowedValue, 0, "Should report zero borrowed value");
        assertEq(position.collateralValue, 0, "Should report zero collateral value");
    }
    ...
    function testGetUserAccountData_HealthyPosition() public {
        ...
        assertEq(totalCollateralValue, 1350e18, "Should return borrowable collateral value");
        assertEq(totalDebtValue, 500e18, "Should return total debt value");
        
        // Available borrows: 1350 - (500 * 1.5) = 1350 - 750 = 600
        assertEq(availableBorrows, 600e18, "Should calculate available borrows");
        
        assertEq(currentLiquidationThreshold, 0.95e18, "Should return liquidation threshold");
        assertEq(healthFactor, 2.85e18, "Should calculate health factor: (1500*0.95)/500");
    }
    ...
    function testGetUserAccountData_MultiAsset() public {
        ...
        assertEq(totalCollateralValue, 2500e18, "Should calculate multi-asset borrowable collateral");
        assertEq(totalDebtValue, 800e18, "Should return debt value");
        
        // Available: 2500 - (800 * 1.5) = 2500 - 1200 = 1300
        assertEq(availableBorrows, 1300e18, "Should calculate available borrows");
        
        // Weighted threshold: (1000*0.95 + 2000*0.85) / 3000 = (950 + 1700) / 3000 = 0.883...
        assertApproxEqAbs(currentLiquidationThreshold, 0.883333333333333333e18, 1e15, "Should calculate weighted threshold");
    }

    function testGetUserAccountData_NoCollateral() public {
        (
            uint256 totalCollateralValue,
            uint256 totalDebtValue,
            uint256 availableBorrows,
        ...
        assertEq(currentLiquidationThreshold, 0, "Should have zero liquidation threshold");
        assertEq(healthFactor, type(uint256).max, "Should have max health factor");
    }
    ...
    function testGetMaxWithdrawCapacity_NoDebt() public {
        ...
        assertEq(maxWithdraw, 1000e6, "Should allow full withdrawal with no debt");
    }
    ...
    function testGetMaxWithdrawCapacity_WithDebt() public {
        ...
        assertApproxEqAbs(maxWithdraw, 736e6, 2e6, "Should calculate max safe withdrawal");
    }
    ...
    function testGetMaxWithdrawCapacity_NoSupply() public {
        uint256 contractBalance = 1000e6;
    ...
    function testGetPositionHealthMetrics_HealthyPosition() public {
        ...
        assertApproxEqAbs(healthFactor, 3167e15, 1e15, "Should calculate health factor");
        ...
        uint256 expectedLiquidationPrice = PRECISION - (1.2e18 * PRECISION) / healthFactor;
        assertEq(liquidationPrice, expectedLiquidationPrice, "Should calculate liquidation price drop");
    }
    ...
    function testGetPositionHealthMetrics_RiskyPosition() public {
        ...
        assertTrue(healthFactor < 1.5e18 && healthFactor >= 1.2e18, "Should be risky health factor");
    ...
    function testGetPositionHealthMetrics_LiquidatablePosition() public {
        ...
        assertTrue(healthFactor < 1.2e18, "Should be below liquidation threshold");
        assertEq(healthStatus, 2, "Should be liquidatable status");
        assertTrue(improvementNeeded > 0, "Should need significant improvement");
    }
    ...
    function testGetActiveAssets_MultiplePositions() public {
        ...
        assertTrue(foundUsdc && foundEth && foundBtc, "Should find all active assets");
    }

    function testGetActiveAssets_NoPositions() public {
        (
            address[] memory activeAssets,
            bool[] memory hasSupply,
            bool[] memory hasBorrow
    ...
    function testIntegration_CompletePositionAnalysis() public {
        ...
        // Verify totals
        assertEq(positionData.totalCollateralValue, 14000e18, "Should sum all collateral: 5000+4000+5000");
        assertEq(positionData.totalDebtValue, 5000e18, "Should sum all debt: 3000+2000");
        
        // Verify arrays
        assertEq(positionData.suppliedAssets.length, 3, "Should have three supplied assets");
        assertEq(positionData.borrowedAssets.length, 2, "Should have two borrowed assets");
        ...
        assertEq(totalDebtValue, positionData.totalDebtValue, "Debt values should match");
        assertEq(healthFactor, positionData.healthFactor, "Health factors should match");
    }
    ...
    function testEdgeCase_VerySmallAmounts() public {
        ...
        assertTrue(positionData.totalCollateralValue > 0, "Should handle very small collateral");
        assertTrue(positionData.totalDebtValue > 0, "Should handle very small debt");
        assertTrue(positionData.healthFactor < type(uint256).max, "Should calculate finite health factor");
    }
    ...
    function testEdgeCase_LargeAmounts() public {
        ...
        assertTrue(positionData.totalCollateralValue > 0, "Should handle large collateral");
        assertTrue(positionData.totalDebtValue > 0, "Should handle large debt");
        assertTrue(positionData.healthFactor > MINIMUM_HEALTH_FACTOR, "Should maintain healthy position");
    }
}