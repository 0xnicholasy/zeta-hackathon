...
contract CoreCalculationsTest is Test {
    ...
    function testNormalizeToDecimals_6To18() public pure {
    ...
    function testNormalizeToDecimals_8To18() public pure {
    ...
    function testNormalizeToDecimals_18To18() public pure {
    ...
    function testNormalizeToDecimals_24To18() public pure {
    ...
    function testDenormalizeFromDecimals_18To6() public pure {
        uint256 normalizedAmount = 1000 * 1e18;
        uint256 denormalized = CoreCalculations.denormalizeFromDecimals(
            normalizedAmount,
            6
        );
    ...
    function testDenormalizeFromDecimals_18To8() public pure {
    ...
    function testDenormalizeFromDecimals_18To18() public pure {
    ...
    function testCalculateAssetValue_USDC() public view {
        uint256 amount = 1000 * 1e6; // 1000 USDC
        uint256 price = 1 * 1e18; // $1.00
        uint256 value = CoreCalculations.calculateAssetValue(
            amount,
            address(mockToken6),
            price
        );
        assertEq(
            value,
            1000 * 1e18,
            "Should calculate correct USD value for USDC"
        );
    }

    function testCalculateAssetValue_ETH() public view {
        ...
        assertEq(
            value,
            4000 * 1e18,
            "Should calculate correct USD value for ETH"
        );
    }

    function testCalculateAssetValue_BTC() public view {
        ...
        assertEq(
            value,
            50000 * 1e18,
            "Should calculate correct USD value for BTC"
        );
    }

    function testCalculateAssetValue_ZeroAmount() public view {
        uint256 value = CoreCalculations.calculateAssetValue(
            0,
            address(mockToken18),
            2000 * 1e18
    ...
    function testCalculateAssetValue_ZeroPrice() public view {
        uint256 value = CoreCalculations.calculateAssetValue(
            1e18,
            address(mockToken18),
            0
    ...
    function testCalculateAssetAmountFromValue_USDC() public view {
        uint256 usdValue = 1000 * 1e18; // $1000
        uint256 price = 1 * 1e18; // $1.00
        uint256 amount = CoreCalculations.calculateAssetAmountFromValue(
            usdValue,
            address(mockToken6),
            price
        );
        assertEq(
            amount,
            1000 * 1e6,
            "Should calculate correct USDC amount from USD value"
        );
    }

    function testCalculateAssetAmountFromValue_ETH() public view {
        ...
        assertEq(
            amount,
            2 * 1e18,
            "Should calculate correct ETH amount from USD value"
        );
    }

    function testCalculateAssetAmountFromValue_BTC() public view {
        ...
        assertEq(
            amount,
            1 * 1e8,
            "Should calculate correct BTC amount from USD value"
        );
    }

    function testCalculateAssetAmountFromValue_ZeroValue() public view {
        uint256 amount = CoreCalculations.calculateAssetAmountFromValue(
            0,
            address(mockToken18),
            2000 * 1e18
    ...
    function testCalculateAssetAmountFromValue_ZeroPrice() public view {
        uint256 amount = CoreCalculations.calculateAssetAmountFromValue(
            1000 * 1e18,
            address(mockToken18),
            0
    ...
    function testValidatePrice_ValidPrices() public pure {
        assertTrue(
            CoreCalculations.validatePrice(MIN_VALID_PRICE),
            "Should validate minimum price"
        );
        assertTrue(
            CoreCalculations.validatePrice(1000 * 1e18),
            "Should validate normal price"
        );
        assertTrue(
            CoreCalculations.validatePrice(MAX_VALID_PRICE),
            "Should validate maximum price"
        );
    }

    function testValidatePrice_InvalidPrices() public pure {
        assertFalse(
            CoreCalculations.validatePrice(0),
            "Should reject zero price"
        );
        assertFalse(
            CoreCalculations.validatePrice(MIN_VALID_PRICE - 1),
            "Should reject price below minimum"
        );
        assertFalse(
            CoreCalculations.validatePrice(MAX_VALID_PRICE + 1),
    ...
    function testSafeMulDiv_NormalOperation() public pure {
        uint256 a = 1000 * 1e18;
        uint256 b = 2 * 1e18;
    ...
    function testSafeMulDiv_ZeroValues() public pure {
        assertEq(
            CoreCalculations.safeMulDiv(0, 1e18),
            0,
            "Should return zero for zero first value"
        );
        assertEq(
            CoreCalculations.safeMulDiv(1e18, 0),
            0,
            "Should return zero for zero second value"
        );
    }
    ...
    function testSafeDivMul_NormalOperation() public pure {
        uint256 a = 1000 * 1e18;
        uint256 b = 2 * 1e18;
    ...
    function testSafeDivMul_ZeroNumerator() public pure {
    ...
    function testCalculatePercentage_NormalOperation() public pure {
        uint256 value = 1000 * 1e18;
        uint256 percentage = 1e17; // 10% in 18-dec fixed point
        uint256 result = CoreCalculations.calculatePercentage(
            value,
            percentage
        );
    ...
    function testCalculatePercentage_HighPercentage() public pure {
        uint256 value = 1000 * 1e18;
        uint256 percentage = 105e16; // 105% in 18-dec fixed point
        uint256 result = CoreCalculations.calculatePercentage(
            value,
            percentage
        );
    ...
    function testCalculatePercentage_ZeroValues() public pure {
        assertEq(
            CoreCalculations.calculatePercentage(0, 0.1 * 1e18),
            0,
            "Should return zero for zero value"
        );
        assertEq(
            CoreCalculations.calculatePercentage(1000 * 1e18, 0),
            0,
            "Should return zero for zero percentage"
        );
    }
    ...
    function testEqualWithinTolerance_ExactMatch() public pure {
        uint256 a = 1000 * 1e18;
        uint256 b = 1000 * 1e18;
        assertTrue(
            CoreCalculations.equalWithinTolerance(a, b, 1e15),
            "Should return true for exact match"
        );
    }

    function testEqualWithinTolerance_WithinTolerance() public pure {
        uint256 a = 1000 * 1e18;
        uint256 b = 1000 * 1e18 + 1e15; // 0.001 difference
        assertTrue(
            CoreCalculations.equalWithinTolerance(a, b, 1e16),
            "Should return true within tolerance"
        );
    }

    function testEqualWithinTolerance_OutsideTolerance() public pure {
        uint256 a = 1000 * 1e18;
        uint256 b = 1000 * 1e18 + 1e16; // 0.01 difference
        assertFalse(
            CoreCalculations.equalWithinTolerance(a, b, 1e15),
            "Should return false outside tolerance"
        );
    }

    function testEqualWithinTolerance_ReverseDifference() public pure {
        uint256 a = 1000 * 1e18 + 1e15;
        uint256 b = 1000 * 1e18;
        assertTrue(
            CoreCalculations.equalWithinTolerance(a, b, 1e16),
            "Should handle reverse difference"
        );
    }

    // ==================== Asset Decimals Tests ====================

    function testGetAssetDecimals_StandardTokens() public view {
        assertEq(
            CoreCalculations.getAssetDecimals(address(mockToken6)),
            6,
            "Should return 6 decimals for USDC-like"
        );
        assertEq(
            CoreCalculations.getAssetDecimals(address(mockToken18)),
            18,
            "Should return 18 decimals for ETH-like"
        );
        assertEq(
            CoreCalculations.getAssetDecimals(address(mockToken8)),
            8,
            "Should return 8 decimals for BTC-like"
        );
    }

    function testGetAssetDecimals_InvalidAddress() public view {
        // Should default to 18 for invalid address
        uint8 decimals = CoreCalculations.getAssetDecimals(address(0));
        assertEq(
            decimals,
            18,
            "Should default to 18 decimals for invalid address"
        );
    }

    // ==================== Weighted Average Tests ====================

    function testCalculateWeightedAverage_NormalCase() public pure {
        ...
        uint256 result = CoreCalculations.calculateWeightedAverage(
            values,
            weights
        );
    ...
    function testCalculateWeightedAverage_EmptyArrays() public pure {
    ...
    function testCalculateWeightedAverage_ZeroWeights() public pure {
    ...
    function testPrecisionHandling_SmallAmounts() public view {
    ...
    function testPrecisionHandling_LargePrices() public view {
    ...
    function testRoundingBehavior() public pure {
        ...
        assertEq(
            denormalized,
            amount,
            "Should maintain precision in round-trip conversion"
        );
    }

    // ==================== Integration Tests ====================

    function testIntegration_FullCalculationCycle() public view {
        // Test a complete calculation cycle: amount -> normalized -> value -> back to amount
        uint256 originalAmount = 1000 * 1e6; // 1000 USDC
        uint256 price = 1 * 1e18; // $1.00

        // Calculate USD value
        uint256 usdValue = CoreCalculations.calculateAssetValue(
            originalAmount,
            address(mockToken6),
            price
        );
        ...
        uint256 calculatedAmount = CoreCalculations
            .calculateAssetAmountFromValue(
                usdValue,
                address(mockToken6),
                price
            );
        ...
        assertEq(
            calculatedAmount,
            originalAmount,
            "Should maintain precision in full calculation cycle"
        );
    }

    function testIntegration_CrossDecimalCalculations() public view {
        ...
        uint256 equivalentUsdc = CoreCalculations.calculateAssetAmountFromValue(
            usdValue,
            address(mockToken6),
            usdcPrice
        );
        assertEq(
            equivalentUsdc,
            4000 * 1e6,
            "Should calculate equivalent USDC amount"
        );
    }
}
