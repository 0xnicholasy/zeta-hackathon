...
import { formatEther, formatUnits, parseUnits } from "ethers/src.ts/utils";
...
async function main() {
  ...
  if (!fs.existsSync(deploymentFile)) {
    console.error(`Deployment file ${deploymentFile} not found. Please run simple deployment and initialization first.`);
    process.exit(1);
  }
  ...
  const simpleLendingProtocol = await ethers.getContractAt("SimpleLendingProtocol", deploymentInfo.contracts.SimpleLendingProtocol);
  const simplePriceOracle = await ethers.getContractAt("SimplePriceOracle", deploymentInfo.contracts.SimplePriceOracle);
  ...
  if (canBorrow) {
    try {
      ...
      console.log("User1 USDC balance after borrow:", formatUnits(user1UsdcBalance, 6));

      const healthFactorAfter = await simpleLendingProtocol.getHealthFactor(user1.address);
      console.log("User1 health factor after borrow:", formatUnits(healthFactorAfter, 0));
  ...
  console.log("User1 health factor after repay:", formatUnits(healthFactorAfterRepay, 0));
  ...
  console.log("User1 health factor after price drop:", formatUnits(healthFactorAfterPriceDrop, 0));
  ...
  if (isLiquidatable) {
    ...
    try {
      ...
      console.log("User1 health factor after liquidation:", formatUnits(healthFactorAfterLiquidation, 0));
  ...
  if (canWithdraw) {
    try {
      ...
      console.log("User1 ETH balance after withdrawal:", formatEther(user1EthBalanceAfterWithdraw));
